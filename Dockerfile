...
# Builds: Custom Kernel, Initramfs, Rootfs, QEMU (Android ARM64)
# ─────────────────────────────────────────────────────────────────────────────
...
COPY kernel.config /tmp/stryker_kernel.config
# Force-builtin fragment: netfilter, WiFi, USB — all built-in for instant availability
RUN printf '%s\n' \
    'CONFIG_IPV6=y' \
    'CONFIG_BRIDGE=y' \
...
RUN cd linux-${KERNEL_VERSION} \
    ...
    && ./scripts/kconfig/merge_config.sh -m .config /tmp/stryker_kernel.config \
    && ./scripts/kconfig/merge_config.sh -m .config /tmp/forced_builtin.config \
    && make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- olddefconfig \
    && echo "=== verifying critical options are =y ===" \
    && for opt in IPV6 BRIDGE NF_TABLES NF_NAT \
                  VETH TUN OVERLAY_FS FUSE_FS EXT4_FS_SECURITY \
                  SQUASHFS_ZSTD BINFMT_MISC \
                  VSOCKETS VIRTIO_VSOCKETS \
                  USB USB_SUPPORT USB_XHCI_HCD USB_STORAGE \
                  SCSI BLK_DEV_SD; do \
           grep -q "^CONFIG_${opt}=y$" .config \
               || { echo "FATAL: CONFIG_${opt} is not =y" >&2; exit 1; }; \
       done \
    && echo "=== all critical options built-in ==="
...
RUN mkdir -p /output \
    && cp linux-${KERNEL_VERSION}/arch/arm64/boot/Image.gz /output/Image
...
# ==============================================================================
# SECTION 2: Rootfs (Debian Trixie with pentest tools)
# ==============================================================================

FROM debian:trixie AS rootfs-packer
...
# API-26 Bionic lacks iconv symbols in libc. Tiny shim.
...
# libusb — required for QEMU's usb-host device
...
# Disable ivshmem (needs shm_open which is not in Bionic)
...
# shm_open/shm_unlink shim (absent from NDK API-26 stubs)
...
# coroutine sigsetjmp shim — PAC-free for Pixel 10 (Tensor G6) SIGILL fix
...
# Patch coroutine-ucontext.c for PAC-free jmp
...
# USB passthrough: skip libusb device enumeration (Android unprivileged)
RUN sed -i 's@^    rc = libusb_init(&ctx);@#if defined(__ANDROID__)\n    libusb_set_option(NULL, LIBUSB_OPTION_NO_DEVICE_DISCOVERY); /* skip enumeration */\n#endif\n    rc = libusb_init(\&ctx);@' ${QEMU_DIR}/hw/usb/host-libusb.c \
...
# Bridge (PTY <-> virtio-console relay)
...
# Launcher (PR_SET_PDEATHSIG wrapper)
...
# Rename QEMU binary for Android naming convention
...
# ==============================================================================
# SECTION 4: Final Artifacts Stage
# ==============================================================================

FROM scratch AS final
# Kernel
COPY --from=kernel-builder /output/Image /Image
# Initramfs
COPY --from=packer /output/initrd.img /initrd.img
# Rootfs
COPY --from=rootfs-packer /work/rootfs.imgz /rootfs.imgz
# QEMU + helpers
