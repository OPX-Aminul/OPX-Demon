...
# dull prompt this replaces. We brand the prompt "⚡OPX-Demon" instead.
...
#   ┌──(⚡OPX-Demon)─[ ~/current/path ]
#   └─#
PS1="${__s_rail}┌──${__s_rst}${__s_dim}(${__s_rst}${__s_brand}⚡OPX-Demon${__s_rst}${__s_dim})${__s_rst}${__s_dim}─[${__s_rst}${__s_path}\\w${__s_rst}${__s_dim}]${__s_rst}\n${__s_rail}└─${__s_rst}${__s_tip} "
...
if [ -z "$__OPX_DEMON_GREETED" ]; then
    __OPX_DEMON_GREETED=1
    printf '\033[1;38;5;39m⚡ OPX-DEMON\033[0m \033[2m- pentest shell\033[0m\n'
fi
