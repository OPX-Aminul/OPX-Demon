package com.opx.demon.localnetwork;
...
import com.opx.demon.exploithub.utils.BasicExploitLaunch;
import com.opx.demon.localnetwork.utils.NmapReportGenerator;
import com.opx.demon.metasploit.InstallMetasploit;
import com.opx.demon.utils.AdvancedProcess;
import com.opx.demon.utils.AdvancedProcessList;
...
public class LocalAdapter extends RecyclerView.Adapter<LocalAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder adapter, @SuppressLint("RecyclerView") final int position) {
        ...
        if (vendorText == null || vendorText.trim().isEmpty()) {
            vendorText = com.opx.demon.localnetwork.utils.MacLine.stripMac(device.getVendor());
        }
        ...
        new Thread(() -> {
            ...
            activity.runOnUiThread(() -> {
                if (installed) {
                    ...
                    com.opx.demon.hydra.HydraDialog.show(context, activity, core, device);
                    return;
                }
                FragmentManager manager = ((AppCompatActivity) context).getSupportFragmentManager();
                manager.beginTransaction()
                        .replace(R.id.flContent, new com.opx.demon.hydra.HydraInstall())
                        .commit();
            });
        }, "hydra-check").start();
    }
    ...
    private void launchCameradar(String ip) {
        new Thread(() -> {
            ...
            activity.runOnUiThread(() -> {
                ...
                FragmentManager manager = ((AppCompatActivity) context).getSupportFragmentManager();
                manager.beginTransaction()
                        .replace(R.id.flContent, new com.opx.demon.cameradar.CameradarInstall())
                        .commit();
            });
    ...
    private void generatePayload() {
        com.opx.demon.metasploit.PayloadGenerator.show(context, activity, core);
    }
            ...
            msfpc[0] = new AdvancedProcess(activity, context, "msfpc " + payload.getText().toString() + " " + ip.getText().toString() + " " + port.getText().toString(), true) {
                ...
                public void onNewLine(String line) {
                    if (line.contains("created:")) {
                        new Thread(() -> {
                            core.createFolder("/sdcard/OPX-Demon/payloads");
                            String path = line.replaceAll(".*:", "").replace("'[01;33m//", "").replace("[00m'", "").replaceAll("\\s+", "").trim();
                            core.customChrootCommand("cp " + path + " /sdcard/OPX-Demon/payloads/" + path.split("/")[path.split("/").length - 1].replaceAll(".*\\.", name.getText().toString() + "."));
                        }).start();
                    }
                }
