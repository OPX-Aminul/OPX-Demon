package com.opx.demon.metasploit.utils;
...
public class MsfRpcConsole {
    ...
    public ArrayList<String> command(String cmd, long timeoutMs) {
        ...
        synchronized (ioLock) {
            String sentinel = "__OPX_DEMON_END_" + Core.generateString().substring(0, 12) + "__";
            try {
                stdin.write((cmd + "\n").getBytes());
    ...
    static boolean shouldSuppress(String line) {
        ...
        if (line.contains("__OPX_DEMON_END_")) return true;
        ...
        if (line.startsWith("echo __OPX_DEMON_END_")) return true;
        return false;
    }
