...
import com.opx.demon.about.AboutFragment;
import com.opx.demon.appintro.AppIntroActivity;
import com.opx.demon.coremanger.CoreManager;
import com.opx.demon.custom.Device;
import com.opx.demon.custom.WiFINetwork;
import com.opx.demon.dashboard.Dashboard;
import com.opx.demon.arsenal.ArsenalFragment;
import com.opx.demon.geomac.GeoMac;
import com.opx.demon.handshakes.HandshakeStorage;
import com.opx.demon.localnetwork.LocalMain;
import com.opx.demon.macchanger.MACChangerFragment;
import com.opx.demon.metasploit.InstallMetasploit;
import com.opx.demon.metasploit.utils.MetasploitUtils;
import com.opx.demon.nmap.NmapScanner;
import com.opx.demon.nuclei.InstallNuclei;
import com.opx.demon.ota.UpdateManager;
import com.opx.demon.settings.SettingsNew;
import com.opx.demon.utils.Core;
import com.opx.demon.utils.PromoDialogs;
import com.opx.demon.hid.ui.HidFragment;
import com.opx.demon.usbarsenal.ui.UsbArsenalFragment;
import com.opx.demon.vnc.VNCFragment;
import com.opx.demon.wifi.Wifi;
import com.opx.demon.wpair.WpairFragment;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        core = new Core(this);
        com.opx.demon.logger.LogStore.from(getApplicationContext());
        ...
        if (core.getBoolean("clean_after_update")) {
            new Thread(() -> core.deleteFile("/data/data/com.opx.demon/files/OPX-Demon.apk")).start();
    ...
    private void refreshEngineStatus() {
        ...
        if (!com.opx.demon.engine.EngineType.isChosen(core)) {
            vmReady = false;
        ...
        new Thread(() -> {
            ...
            com.opx.demon.engine.EngineStatus es =
                    com.opx.demon.engine.EngineStatus.current(core, mounted, rootless);
            runOnUiThread(() -> {
    ...
    private void usbDialog() {
        ...
        com.opx.demon.netdetect.UsbDialogRenderer renderer =
                new com.opx.demon.netdetect.UsbDialogRenderer(usbDialog);
    ...
    private void scanUsb(com.opx.demon.netdetect.UsbDialogRenderer renderer) {
        new Thread(() -> {
            java.util.List<com.opx.demon.netdetect.UsbDeviceReport> devs =
                    com.opx.demon.netdetect.NetDetector.listNetworkUsbDevices(this);
            com.opx.demon.netdetect.UsbDeviceReport pick = devs.isEmpty() ? null : devs.get(0);
            runOnUiThread(() -> {
    ...
    private void startRootlessLaunch() {
        boolean installed = com.opx.demon.engine.RootlessEngine.get(this).isInstalled();
        if (!core.getBoolean("first_open")
                || (!installed && !com.opx.demon.engine.RootlessCoreFiles.anyPresent(this))) {
    ...
    private void runLaunchFlow() {
        ...
        new Thread(() -> {
            ...
            new Thread(() -> {
                ...
                if (core != null) core.chmodFolder("/data/data/com.opx.demon/files");
            }).start();
        }).start();
    }
    ...
    private void copyAssets() {
        ...
        if (files != null) for (String filename : files) {
            ...
            try {
                ...
                File outFile = new File("/data/data/com.opx.demon/files/", filename);
    ...
    public static class Receiver implements MainActivityReceiver {
        ...
        public void changeFragment(int itemId, int enterAnim, int exitAnim) {
            ...
            if (itemId == R.id.terminal_item) {
                Intent terminal = new Intent(settings.getContext(),
                        com.opx.demon.terminal.ui.term.NeoTermActivity.class);
                terminal.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                terminal.putExtra(com.opx.demon.terminal.ui.term.NeoTermActivity.EXTRA_NEW_SESSION, true);
        ...
        private static Fragment fragmentFor(int itemId) {
            ...
            if (itemId == R.id.logs_item) return new com.opx.demon.logger.LoggerFragment();
