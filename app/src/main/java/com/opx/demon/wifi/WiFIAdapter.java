...
import com.opx.demon.MainActivity;
import com.opx.demon.R;
import com.opx.demon.custom.WiFINetwork;
import com.opx.demon.utils.AdvancedProcess;
import com.opx.demon.utils.AdvancedThread;
import com.opx.demon.utils.Core;
import com.opx.demon.utils.MonitorManager;
import com.opx.demon.utils.SimpleProcess;
import com.opx.demon.utils.Utils;
            ...
            pixie = new AdvancedProcess(activity, context, cmd, true) {
                ...
                public void onFinished(ArrayList<String> outputList) {
                    ...
                    if (result.getOK()){
                        ...
                        com.opx.demon.geomac.GeoHooks.recordPixie(
            ...
            handshake = new AdvancedThread(activity,context) {
                ...
                public void doOnBackground() {
                    ...
                    if (monitor && monitor2){
                        ...
                        core.customChrootCommand("mkdir -p /sdcard/OPX-Demon/hs /sdcard/OPX-Demon/captured; "
                                + "rm -f /sdcard/OPX-Demon/hs/handshake*");
                        final String capIface = core.getHSInterface();
                        if (canceled) return;
                        new Thread(() -> {
                            if (canceled) return;
                            String cmd = "airodump-ng " + capIface + " -w /sdcard/OPX-Demon/hs/handshake  --ignore-negative-one --output-format pcap -c "+network.getChannel()+" --bssid " + network.getMac()+" --update 3";
                            if (network.getIs5hhz() && network.getChannel() <= 0){
                                cmd = "airodump-ng " + capIface + " -w /sdcard/OPX-Demon/hs/handshake --ignore-negative-one --output-format pcap  --bssid " + network.getMac() + " --band a --update 3";
                            }
