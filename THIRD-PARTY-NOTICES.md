...
| `linux-uml` | Linux 7.2-rc UML tree (arm64 UML port, upstream `arch/um` + the public [g0l4/uml-for-aarch64](https://github.com/g0l4/uml-for-aarch64) tree) | **GPL-2.0** | User-Mode Linux arm64 kernel binary; built by `build-tools/uml-build.sh` from the pinned UML tree (`UML_REF` in the `Dockerfile`). Corresponding source is the build tree itself, available upstream and on request. |
