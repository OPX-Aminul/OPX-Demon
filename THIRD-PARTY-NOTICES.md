...
| `linux-uml` | Linux 7.2-rc UML tree (arm64 UML port, upstream `arch/um` + the original developer's [zalexdev/linux-um-arm64](https://github.com/zalexdev/linux-um-arm64) `um-arm64` branch — the exact tree behind the released build commit `8897487c5223`) | **GPL-2.0** | User-Mode Linux arm64 kernel binary; built by `build-tools/uml-build.sh` from the pinned UML tree (`UML_REPO`/`UML_REF` in the `Dockerfile`). Corresponding source is the build tree itself, available upstream and on request. |
| `stub_exe` | generated by the arm64 UML kernel build (build-tree stub binary) | **GPL-2.0** | Built together with `linux-uml`; corresponding source as above. |
